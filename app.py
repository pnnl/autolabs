...
from src.st_callable_util import get_streamlit_cb  # Utility function to get a Streamlit callback handler with context
...
if st.session_state.iteration_count == 0:
    ...
    with st.chat_message("system"):
        ...
        with st.form(key='graph_options_form'):
            ...
            gpt_4o_self_checks = st.toggle("use Guided Self-Checks", value=False, key='gpt_4o_self_checks')
            reasoning_self_checks = st.toggle("use Unguided Self-Checks", value=False, key='reasoning_self_checks')
