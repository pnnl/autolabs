    ...
    try:
        ...
        try:
            messages = [
                {
                    "role": "user",
                    "content": f"return only the molecular weight of {chem_name} in g/mol. do not include units in the returned value.",
                }
            ]
...
def get_mol_density(chem_name, completer):
    ...
    messages = [
        {
            "role": "user",
            "content": f"return only the molecular density of {chem_name} in g/cm3. do not include units in the returned value.",
        }
    ]
    response = completer.get_chat_completions(messages)
    # response = model.invoke(messages)
...
def get_chem_state(chem_name, model):
    ...
    messages = [
        SystemMessage(
            f"you are an expert chemist. if {chem_name} is in solid form at room temperature return 1 else 0. just return the number."
        )
    ]
    response = model.invoke(messages)
...
@tool
def get_chem_volume(chem_name: str, chem_weight: float):
    """
...
def find_the_concentration_of_n_percent_solution(
    ...
    n = float(
        model.invoke(
            [
                HumanMessage(
                    f"What is the weight percentage of this chemical. chemical = {chem_name}. return only the percentage."
                )
            ]
        ).content
    )
...
def find_the_concentration_of_n_percent_solution(
    ...
    n = float(
        model.invoke(
            [
                HumanMessage(
                    f"What is the weight percentage of this chemical. chemical = {chem_name}. return only the percentag without % symbol."
                )
            ]
        ).content
    )
...
def find_stock_solution_volume_corresponding_to_a_modifier(
    ...
    modifier_concentration: Annotated[
        str, "concentration of the modifier in the final solution"
    ],
):
    """
