...
def rewrite_prompt(prompt, completer):
    ...
    messages=[
            {"role": "system", "content": 'rewrite the following user prompt in a llm-friendly way'},
...
def save_steps():
    ...
    if '<final-steps>' in final_steps_message and  '</final-steps>' in final_steps_message:
        with open('current_steps.pkl', 'wb') as f:
            pickle.dump( {'steps': final_steps_message,
                          'array_dim': array_dim
                          }, f)
