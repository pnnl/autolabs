...
def do_self_checks_reasoning(messages, assistant_response):
    ...
    while modify and valid_count < 5:
        ...
        modify, reasoning, assistant_response = validate(
            chat_history, assistant_response
        )
        # chat_history[-1]['role'] = 'assistant'
        chat_history[-1].type = "ai"
        chat_history[-1].content = (
            chat_history[-1]
            .content.replace("<final-steps>", "")
            .replace("</final-steps>", "")
        )
        chat_history.append(AIMessage(reasoning))
        chat_history.append(AIMessage(assistant_response))
...
Undesrand_And_Refine_Experiment_agent = create_react_agent(
    llm_o3_medium,
    tools=[],
    prompt="""Your role is to understand, refine and correct experimental steps according to user instructions. 
...
def understand_and_refine_node(state: State) -> Command[Literal["__end__"]]:
    ...
    if st.session_state.use_processing_steps_rag:
        ...
        state["messages"].append(
            SystemMessage(
                f"Below are some experiment objectives and corresponding steps from past experiment for reference. Use them to determine the processing steps for the current experiment. \n\n {step_context}"
            )
        )
    ...
    if (
        "<final-steps>" in result["messages"][-1].content
        and "</final-steps>" in result["messages"][-1].content
    ):
        logger.info(f"final steps were detected at the {node} node")

        if st.session_state.use_self_checks:
            logger.info(
                "Using self checks. Sending the final steps to self-checks node."
            )
            return Command(
                update={
                    "messages": [
            ...
            logger.info(
                "Using reasoning self checks. Sending the final steps to self-checks-reasoning node."
            )
            return Command(
                update={
                    "messages": [
            ...
            update={
                "messages": [
                    AIMessage(
                        ...
                        name="Undesrand_And_Refine_Experiment",
                    )
                ]
            },
            goto="reflection",
...
def reflection_node(
    state: State,
) -> Command[Literal["__end__", "Undesrand_And_Refine_Experiment"]]:
    ...
    """
    critique_prompt = (
        common_critique_prompt
        + """If the description is not clear enough, respond with 'no'.
...
"""
    ...
    critique_prompt2 = (
        common_critique_prompt
        + """If the description is not clear, come up with recommendations to improve clarity.
"""
    )
    ...
    if eval_result == "yes" or st.session_state.reflection_count > 1:
        ...
        with st.sidebar:
            st.info(
                f"number of time the reflection node is visited {st.session_state.reflection_count}"
            )
        logger.info(
            f"number of time the reflection node is visited {st.session_state.reflection_count}"
        )
        ...
        return Command(
            update={
                "messages": [HumanMessage(content=resp.content, name="Reflection")],
            },
...
Determine_Chemicals_And_Format_agent = create_react_agent(
    llm_o3_medium,
    tools=[],
    prompt="""Your role is to determine available source chemicals and their formats. For example the chemicals may be solids, 
    ...
    return Command(
        update={
            "messages": [
                AIMessage(
                    content=result["messages"][-1].content,
                    name="Determine_Chemicals_And_Formats",
                )
            ]
        },
        goto="__end__",
...
Determine_Reactions_agent = create_react_agent(
    llm_o3_medium,
    tools=[],
    prompt="""Your role is to determine which reactions, mixtures, or solutions will be performed. 
...
def reactioons_node(state: State) -> Command[Literal["__end__"]]:
    ...
    return Command(
        update={
            "messages": [
                AIMessage(
                    content=result["messages"][-1].content, name="Determine_Reactions"
                )
            ]
        },
        goto="__end__",
...
def calculation_node(state: State) -> Command[Literal["__end__"]]:
    ...
    Calculate_Chemical_Amounts_For_Reactions_agent = create_react_agent(
        llm_o3_medium, tools=tools, prompt=calculation_prompt
    )
    ...
    if st.session_state.redo_calculations and st.session_state.n_redone < 2:
        logger.info(
            f"Redoing calculations - with tools. n_redone = {st.session_state.n_redone}"
        )
...
"""
        output = llm_o3_medium.with_structured_output(Compare_Calculations).invoke(
            messages
        )
        ...
        if output["result"] == "yes":
            return Command(
                update={
                    ...
                    + [
                        AIMessage(
                            content=result["messages"][-1].content,
                            name="Calculate_Chemical_Amounts_For_Reactions",
                        )
                    ]
                },
                goto="__end__",
            ...
            return Command(
                update={
                    "messages": [
                        HumanMessage(
                            content="How did you do the calculations? Can you rethink about the calculations and tools you selected?",
                            name="Calculate_Chemical_Amounts_For_Reactions",
                        )
                    ]
                },
...
Determine_Vial_Organization_agent = create_react_agent(
    llm_o3_medium,
    tools=[],
    prompt="""Your role is to determine which chemicals should be added to
    ...
    return Command(
        update={
            "messages": [
                AIMessage(
                    content=result["messages"][-1].content,
                    name="Determine_Vial_Organization",
                )
            ]
        },
        goto="__end__",
...
Determine_Processing_Steps_agent = create_react_agent(
    llm_o3_medium,
    tools=[],
    prompt="""Your role is to determine determine the additional processing steps needed.
    ...
    return Command(
        update={
            "messages": [
                AIMessage(
                    content=result["messages"][-1].content,
                    name="Determine_Processing_Steps",
                )
            ]
        },
        goto="__end__",
...
Generate_Final_Steps_agent = create_react_agent(
    llm_o3_medium,
    tools=[],
    prompt="""Your role is to properly order the steps and present them in the format 
