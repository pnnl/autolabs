...
def correct_vial_dims(chat_history, assistant_response, completer):
    ...
    reminder_query = {
        "role": "system",
        "content": f"""Reminder: The dimensions of the array depends on the size of the vials used.
...
def refine_dict(chat_history, assistant_response, completer):
    ...
    # current_history
    reminder_query = SystemMessage(
        f"""Reminder: The dictionary in each <step> should contain key: value pairs. 
...
def refine_units(chat_history, model):
    ...
    reminder_query = SystemMessage(
        f"""Reminder: The units for solid additions should be in mg and the units for liquid additions should be in ul.
                      {common_prompt}"""
    )
...
def refine_efficiency(chat_history, model):
    ...
    reminder_query = SystemMessage(
        f"""Reminder: Please add each input chemical using as few steps as possible.
                      {common_prompt}"""
    )
    chat_history.append(reminder_query)
...
def refine_delays(chat_history, model):
    ...
    reminder_query = SystemMessage(
        f"""Reminder: Please remember to add a sufficient delay for any heating, stirring, and vortexing steps.
                      {common_prompt}"""
    )
    chat_history.append(reminder_query)
...
def refine_plates(chat_history, model):
    ...
    reminder_query = {
        "role": "system",
        "content": f"""Reminder: Please remember to include the plate name in each step. If there are multiple plates being used, please make sure to use the format "Plate x", where x is the plate number.
                      {common_prompt}""",
    }
    chat_history.append(reminder_query)
...
def refine_transfer(chat_history, model):
    ...
    reminder_query = {
        "role": "system",
        "content": f"""Reminder: In steps that involve Transfer, you must include whether it is Uniform or Discrete. Uniform is when the amount being transfered is the same. Discrete is when the amount being transfered is different. Be sure to also include the units of the transfer in the step description.
                      {common_prompt}""",
    }
    chat_history.append(reminder_query)
...
def refine_additions(chat_history, model):
    reminder_query = {
        "role": "system",
        "content": f"""Reminder: In steps that involve chemical additions, each addition must be in its own step. Do not include multiple chemical additions in the same step.
                      {common_prompt}""",
    }
    chat_history.append(reminder_query)
...
def refine_solvents(chat_history, model):
    ...
    reminder_query = SystemMessage(
        f"""Reminder: Please remember to include the type of solvent that is being used. 
                      {common_prompt}"""
    )
    chat_history.append(reminder_query)
